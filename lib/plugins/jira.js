...
function postJiraIssue(slack, redis, data, issues) {
  ...
  promise.allSettled(issues).then(function(results) {
    ...
    if (errors.length > 0) {
      ...
      slack.sendMsg(data.channel, '<@' + data.user + '>: I was unable to retrieve ' + errorMessage + '.');
    }
  });
}

function postSprintStatus(slack, redis) {
...
function disableJiraStatus(slack, redis, data) {
  promise.ninvoke(redis, 'lrange', 'jirastatus', 0, -1).then(function(statuses) {
    if (statuses.length === 0) {
...
function enableJiraStatus(slack, redis, data, viewId) {
  promise.ninvoke(redis, 'lrange', 'jirastatus', 0, -1).then(function(statuses) {
    if (statuses.length !== 0) {
      var found = false;
      statuses.forEach(function(status) {
...
module.exports.getTriggers = function() {
  return [
    ...
    {
      regex: /^<@U08TS076K>(?::)? status off$/g,
      command: disableJiraStatus,
    },
    {
      regex: /^<@U08TS076K>(?::)? status on (\d+)$/g,
